...
def particlefilter_tf(yi,Xi,Xi_1,theta,Xianc,wi,n,lh,innov):
    # innovate
    ...
    loglh = lh(Xi,yi)
    #tf.cond(tf.math.logical_not(tf.math.is_finite(loglh).all())),
    #    lambda: print("weights not finite"),
    ...
    lse = tf.math.reduce_logsumexp(loglh) 
    logml = lse - tf.math.log(n)
    #print("log marginal likelihood = {}".format(logml))
    wi[:] = tf.math.exp(loglh - lse) # normalise weights
    #tf.cond(tf.math.reduce_sum(wi) == 0,
    #    print("Divide by zero sum of weights")
    #    #sys.exit(0)
    #print("n = {}, ess = {}".format(n,1./sum(wi**2)))
    Xianc[:] = tf.cond( 1./tf.math.reduce_sum(tf.math.square(wi)) < 0.5*n,
        #print("Resampling, ess = {}".format(1./sum(wi**2)))
        lambda: resample_cpu(wi),
        lambda: Xianc[:]
    )
    Xi[:].assign(tf.gather(Xi,Xianc)) #Xi[:]=Xi[Xianc,:]
    return logml

def tf_diff(a,firstelcomp):
    """
    Diff along axis 0
...
def resample_tf(weights):
    n = tf.shape(weights)[0]
    indices = tf.zeros_like(weights,dtype=int32)
    #C = [0.] + [sum(weights[:i+1]) for i in range(n)]
    u0, j = tf.random.uniform(), 0
    C = tf.math.cumsum(weights) * n - u0
    changes,idx = tf.unique(tf.floor(C))
    diff_changes = tf_diff(changes,-1)
    # now repeat elements in idx by amounts in diff_changes
    return tf.tile(idx,diff_changes)
